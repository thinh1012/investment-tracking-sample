...
export const CloudSyncModal: React.FC<Props> = ({ isOpen, onClose, onRestore, sync }) => {
    const { user, isLoading, error, status, syncKey, setSyncKey, signIn, signUp, signOut, uploadVault, downloadVault, inspectVault, lastSyncTime } = sync;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200 dark:border-slate-800">
                ...
                <div className="p-6">
                    ...
                    {status && (
                        <div className="mb-4 p-3 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-sm rounded-xl flex items-center gap-2 animate-in slide-in-from-top-2">
                    ...
                    {!user ? (
                        ...
                        <div className="space-y-6">
                            ...
                            {user && !syncKey && (
                                <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl flex items-start gap-2 shadow-sm animate-in slide-in-from-top-1">
                                    <AlertTriangle size={16} className="text-indigo-500 mt-0.5 flex-shrink-0" />
                                    <div>
                                        <p className="text-xs font-bold text-indigo-800 dark:text-indigo-300">Auto-Sync Paused</p>
                                        <p className="text-[10px] text-indigo-700 dark:text-indigo-400 mt-0.5">Please enter your private password below to resume background protection.</p>
                                    </div>
                                </div>
                            )}
                            ...
                            {syncKey && (
                                <div className="space-y-2">
                                    ...
                                    {lastSyncTime && (
                                        <p className="text-[9px] text-center text-slate-500 dark:text-slate-500 font-bold uppercase tracking-widest">
                                            Last Recorded Local Sync: {new Date(Number(lastSyncTime)).toLocaleTimeString()}
                                        </p>
                                    )}
                                </div>
                            )}
