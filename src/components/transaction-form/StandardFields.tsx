        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="md:col-span-2">
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">
                    {type === 'INTEREST' ? 'Source Asset / LP' : 'Asset Symbol'}
                </label>
                <input
                    type="text"
                    required
            ...
            {type === 'DEPOSIT' && setIsCompound && paymentMode !== 'CASH' && (
                <div className="md:col-span-2 flex items-center gap-3 bg-indigo-500/5 border border-indigo-500/10 p-3 rounded-xl animate-in fade-in slide-in-from-top-2">
                    ...
                    <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${isCompound ? 'bg-indigo-600' : 'bg-slate-200 dark:bg-slate-700'}`}
                    >
                        <span
                            className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${isCompound ? 'translate-x-6' : 'translate-x-1'}`}
                        />
                    </button>
                </div>
            )}
                        ...
                        <div className="flex-1">
                            <div className="flex justify-between items-center mb-2">
                                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">
                                    {type === 'INTEREST'
                                        ? 'Earned Value (USD)'
                                        : (inputMode === 'LP' && lpFundingMode === 'CUSTOM'
                                            ? 'Fresh Capital'
                                    ...
                                    <div className="flex bg-slate-100 dark:bg-slate-800 rounded-lg p-0.5">
                                        {(['CASH', 'ASSET', 'MIXED'] as const).map(m => (
                                            <button
                                                key={m}
                                                type="button"
                                                onClick={() => { setPaymentMode(m); if (m === 'CASH') setIsCompound?.(false); }}
                            ...
                            <div className="flex gap-2">
                                {type !== 'INTEREST' && (paymentMode === 'ASSET' || paymentMode === 'CASH') && (inputMode !== 'LP' || (inputMode === 'LP' && lpMode === 'TOTAL')) && (
                                    <select
