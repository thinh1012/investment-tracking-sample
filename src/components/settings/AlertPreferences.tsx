...
export const AlertPreferences: React.FC<Props> = ({ settings, setSettings }) => {
    ...
    return (
        <div className="space-y-6 md:col-span-2">
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                <div>
                    <h3 className="font-semibold text-slate-900 dark:text-white">Bitcoin Volatility Sentry</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Scan for sudden $ move (up or down) within a timed window.</p>
                </div>
                <div className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center justify-end gap-6 border-t border-slate-100 dark:border-slate-800/50 pt-3 mt-1">
                        <label className="flex items-center gap-2 cursor-pointer group">
                            ...
                            />
                            <span className="text-[11px] font-bold text-slate-500 group-hover:text-blue-500 transition-colors uppercase tracking-tight">Send to Telegram</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="flex items-center gap-4">
                    ...
                    >
                        <option value={30 * 60 * 1000}>Every 30 mins</option>
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <textarea
                        name="volatilityTemplate"
                        value={settings.volatilityTemplate ?? "{emoji} *BTC VOLATILITY ALERT*\n\nMagnitude: {direction} of ${diff}\nWindow: {window}\n\nPrevious: ${prev}\nCurrent: ${current}"}
                        onChange={(e) => setSettings({ ...settings, volatilityTemplate: e.target.value })}
                        ...
                        placeholder="Available vars: {emoji}, {direction}, {diff}, {window}, {prev}, {current}"
                    />
                    <p className="text-[10px] text-slate-400 mt-1">
                        Variables: <code>{'{emoji}'}</code>, <code>{'{direction}'}</code>, <code>{'{diff}'}</code>, <code>{'{window}'}</code>, <code>{'{prev}'}</code>, <code>{'{current}'}</code>
                    </p>
                </div>
