...
import { Plus, ArrowUp, ArrowDown, ChevronDown, ChevronRight } from 'lucide-react';
...
export const LPFeeTracker: React.FC<LPFeeTrackerProps> = ({ assets, transactions, prices, locale, onAddClaim }) => {
    ...
    const inRangePools = lpData.filter(lp => lp.inRange !== false);
    const outOfRangePools = lpData.filter(lp => lp.inRange === false);
    ...
    const renderRows = (pools: typeof lpData) => pools.map((lp) => (
        <tr key={lp.symbol} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 group">
            ...
            <td className="hidden md:table-cell px-4 py-2 min-w-[120px]">
    ...
    return (
        <div className="space-y-6">
                ...
                <table className="w-full text-sm text-left">
                    <thead className="border-b border-slate-200 dark:border-slate-800 text-slate-400 dark:text-slate-500 text-xs font-medium">
                        <tr>
                            ...
                            <th className="hidden md:table-cell px-4 py-2 cursor-pointer hover:text-slate-700 dark:hover:text-slate-300" onClick={() => handleSort('recoveryPercent')}>
                                <div className="flex items-center gap-1">
                                    Payback {sortKey === 'recoveryPercent' && (sortOrder === 'asc' ? <ArrowUp size={12} /> : <ArrowDown size={12} />)}
                                </div>
                            </th>
                            <th className="px-4 py-2 cursor-pointer hover:text-slate-700 dark:hover:text-slate-300" onClick={() => handleSort('dailyFeeRate')}>
                                <div className="flex items-center gap-1">
                                    $/day {sortKey === 'dailyFeeRate' && (sortOrder === 'asc' ? <ArrowUp size={12} /> : <ArrowDown size={12} />)}
                                </div>
                            </th>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {renderRows(inRangePools)}
                    </tbody>
                </table>
                {inRangePools.length === 0 && outOfRangePools.length === 0 && (
            ...
            {/* Out of Range Pools */}
            {outOfRangePools.length > 0 && (
                <div className="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
                    <button
                        onClick={() => setShowOutOfRange(v => !v)}
                        className="w-full flex items-center justify-between px-4 py-3 text-sm font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/50"
                    >
                        <div className="flex items-center gap-2">
                    ...
                    {showOutOfRange && (
                        <table className="w-full text-sm text-left border-t border-slate-200 dark:border-slate-800">
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 opacity-60">
                                {renderRows(outOfRangePools)}
                            </tbody>
                        </table>
                    )}
                </div>
