                    ...
                    <tr>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => onSort('source')}>
                            <div className="flex items-center gap-1">
                                Source {sortKey === 'source' && (sortOrder === 'asc' ? <ArrowUp size={11} /> : <ArrowDown size={11} />)}
                            </div>
                        </th>
                        <th className="px-4 py-2">Rewards</th>
                        <th className="px-4 py-2 text-right cursor-pointer" onClick={() => onSort('totalValue')}>
                            <div className="flex items-center justify-end gap-1">
                                Value {sortKey === 'totalValue' && (sortOrder === 'asc' ? <ArrowUp size={11} /> : <ArrowDown size={11} />)}
                            </div>
                        </th>
                        <th className="px-4 py-2 text-right cursor-pointer" onClick={() => onSort('roi')}>
                            <div className="flex items-center justify-end gap-1">
                                ROI {sortKey === 'roi' && (sortOrder === 'asc' ? <ArrowUp size={11} /> : <ArrowDown size={11} />)}
                            </div>
                        </th>
                        <th className="px-4 py-2 text-right cursor-pointer" onClick={() => onSort('apr')}>
                            <div className="flex items-center justify-end gap-1">
                                APR {sortKey === 'apr' && (sortOrder === 'asc' ? <ArrowUp size={11} /> : <ArrowDown size={11} />)}
                            </div>
                        </th>
                    </tr>
                </thead>
                                ...
                                <td className="px-4 py-2">
                                    <div className="flex flex-col gap-0.5">
                                        {Object.entries(item.tokens).map(([token, amount]) => (
                                            <span key={token} className="text-xs font-mono text-slate-600 dark:text-slate-400 whitespace-nowrap">
                                                {amount.toLocaleString(locale || 'en-US', { maximumFractionDigits: 4 })} <span className="text-slate-400 dark:text-slate-500">{token}</span>
                                            </span>
                                        ))}
                                    </div>
                                </td>
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            {t.pricePerUnit ? (
                                                                <span className="text-slate-500">
                                                                    <span className="mr-2">@ {formatPrice(t.pricePerUnit)}</span>
