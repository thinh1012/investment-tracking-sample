...
import { TrendingUp, TrendingDown, X, Calculator, Zap, Plus, Activity, ArrowUpDown, ArrowUp, ArrowDown, Bell, BellOff, RefreshCw } from 'lucide-react';
...
export const MarketPicks: React.FC<Props> = ({ prices, priceChanges = {}, priceVolumes = {}, locale, onSimulate, manualPriceSources = {} }) => {
    const { picks, addPick, removePick, historicalData, sparklines, saveManualOpen, refreshPicks, isLoading: isPicksLoading } = useMarketPicks();
    ...
    const handleForceRefresh = async () => {
        notify.info('Refreshing market records...');
        await refreshPicks(true);
        notify.success('Market records updated');
    };
    ...
    const HeaderRow = () => (
        <div className="flex items-center justify-between px-3 py-2 text-[10px] uppercase font-semibold text-slate-400 tracking-wider bg-slate-50/50 dark:bg-slate-800/30 rounded-lg mb-1 gap-2">
            ...
            <div className="flex-1 flex items-center justify-end gap-6 text-right">
                ...
                {onSimulate && <span className="w-10"></span>}
            ...
            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50">
                ...
                <div className="flex items-center gap-1">
                    <button
                        ...
                        disabled={isPicksLoading}
                        className={`p-1.5 rounded-lg text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all ${isPicksLoading ? 'animate-spin opacity-50' : ''}`}
                                ...
                                <div className="space-y-1">
                                    {assets.map(pick => (
                                        <MarketPickRow
                                            ...
                                            priceSource={manualPriceSources?.[pick.symbol]}
                                        />
                                    ))}
                                </div>
                            </div>
