...
export const encryptData = async (data: any, password: string): Promise<string> => {
    ...
    // Optimized: Use a TextDecoder with 'latin1' or a faster binary string conversion
    // the standard btoa(String.fromCharCode(...buffer)) can be very slow for large buffers
    const CHUNK_SIZE = 0x8000; // 32KB chunks
    let binary = '';
    for (let i = 0; i < buffer.length; i += CHUNK_SIZE) {
        binary += String.fromCharCode.apply(null, Array.from(buffer.subarray(i, i + CHUNK_SIZE)));
    }
    return btoa(binary);
};
