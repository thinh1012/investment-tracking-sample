...
interface RescueDataExport {
    transactions: Transaction[];
    manualPrincipal: number | null;
    fundingOffset: number | null;
    watchlist: any[];
    alertSettings: NotificationSettings | null;
    exportDate: string;
    version: string;
    _diagnostics: {
        dbs: DiagnosticDB[];
...
export default function RescueData(): React.ReactNode {
    const [data, setData] = useState<RescueDataExport | null>(null);
    const [error, setError] = useState<string | null>(null);
    const [diagnostics, setDiagnostics] = useState<DiagnosticInfo>({ dbs: [], localStorageKeys: [] });

    useEffect(() => {
        async function loadData() {
            try {
                // Diagnostic: List all DBs
                let dbs: DiagnosticDB[] = [];
                if (window.indexedDB && (window.indexedDB as any).databases) {
                    dbs = await (window.indexedDB as any).databases();
                ...
                const exportData: RescueDataExport = {
                    transactions: txs,
