    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const commonData = {
            ...
            notes: (inputMode === 'LP' && lpMode === 'SPLIT')
                ...
                : notes,
            paymentCurrency: (type === 'SELL' || type === 'WITHDRAWAL' || paymentMode === 'ASSET' || paymentMode === 'CASH') ? paymentCurrency : (paymentMode === 'MIXED' ? mixedCashSymbol : undefined),
            paymentAmount: ((type === 'SELL' || type === 'WITHDRAWAL') && totalSpent) ? parseFloat(totalSpent) : ((paymentMode === 'ASSET' && totalSpent) ? parseFloat(totalSpent) : ((paymentMode === 'CASH' && totalSpent) ? parseFloat(totalSpent) : (paymentMode === 'MIXED' && mixedCashAmount ? parseFloat(mixedCashAmount) : undefined))),
        ...
        if (mode === 'SINGLE') {
            if (inputMode === 'LP') {
                ...
                onSave({
                    ...
                    amount: result.finalCostBasis, // Use cost as amount (total value model)
            ...
            const sellTxId = initialData ? initialData.id : crypto.randomUUID();

            // SELL/WITHDRAWAL with received currency - create linked DEPOSIT for received amount
            if ((type === 'SELL' || type === 'WITHDRAWAL') && paymentCurrency && totalSpent && parseFloat(totalSpent) > 0) {
                // First, save the SELL transaction (reduces sold asset balance)
                onSave({
                    id: sellTxId,
                    assetSymbol: symbol.toUpperCase(),
                    amount: parseFloat(amount) || 0,
                    pricePerUnit: parseFloat(price) || undefined,
                    ...commonData
                });
            ...
            // Regular transaction (no auto-credit needed)
            onSave({
                id: sellTxId,
                assetSymbol: symbol.toUpperCase(),
                amount: parseFloat(amount) || 0,
                // DEPOSIT and SELL both need price per unit
                pricePerUnit: (type === 'DEPOSIT' || type === 'SELL') ? (parseFloat(price) || undefined) : undefined,
                ...commonData
            });
        } else {
            // Batch Mode
