...
import { WatchlistService } from '../services/database/OtherServices';
...
export const useWatchlist = (onRefreshPrices?: (force?: boolean) => Promise<void>) => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                setWatchlist(items);
                // Trigger initial price refresh once we have the symbols
    ...
    const handleDeleteWatchlist = async (id: string) => {
        ...
        if (onRefreshPrices) onRefreshPrices(true);
    };
