...
import { SettingsService } from '../services/database/OtherServices';
...
export const useAutoSync = (
    ...
    const performSync = useCallback(async (reason: string) => {
        ...
        // CLOUD SYNC TOGGLE GUARD
        const savedSettings = await SettingsService.get();
        if (savedSettings && savedSettings.cloudSyncEnabled === false) {
            console.log(`Auto-Sync: Automated sync is DISABLED in settings. Skipping (${reason})`);
            return;
        }
