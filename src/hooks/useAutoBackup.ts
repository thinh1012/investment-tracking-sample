...
import { SettingsService, LogService, WatchlistService, PriceDataService, AssetOverrideService } from '../services/database/OtherServices';
...
export const useAutoBackup = () => {
    ...
    const performBackup = async () => {
        try {
            // 1. Gather Data (Unified via BackupService)
            const { BackupService } = await import('../services/database/BackupService');
            const exportData = await BackupService.createFullBackup();
            ...
            downloadAnchorNode.setAttribute("download", `alpha-vault-full-backup-${dateStr}.json`);
