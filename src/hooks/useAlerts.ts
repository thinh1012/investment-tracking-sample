...
export const useAlerts = (prices: Record<string, number>, assets: Asset[] = []) => {
    ...
    useEffect(() => {
        const checkAlerts = async () => {
            ...
            const newAlerts = alerts.map(alert => {
                ...
                const currentPrice = getPrice(alert.symbol, prices);
            ...
            if (watchlistItems.length > 0) {
                ...
                watchlistItems.forEach((item: any) => {
                    const currentPrice = getPrice(item.symbol, prices);
    ...
    const sendNotification = (
        ...
        const formatPrice = (val: number) => val < 1 ? val.toFixed(8) : val.toLocaleString();

        const isDominance = symbol.endsWith('.D');
        const suffix = isDominance ? '%' : '';
        const targetStr = formatPrice(target) + suffix;
        const currentStr = formatPrice(current) + suffix;
        ...
        if (isDominance) {
            // Remove $ preceding placeholders for dominance tickers
            message = message
                .replace('${target}', targetStr)
                .replace('{target}', targetStr)
                .replace('${price}', currentStr)
                .replace('{price}', currentStr);
        } else {
            message = message
                .replace('{target}', targetStr)
